...
const splitInput = (str: string, result: Item[]) => {
    ...
    if (inner.includes("DR:")) {
        result.push({
            ...
            data: inner.slice(3, inner.length).split(";").map(item => item.trim())
        });
        return true;
    }
    ...
    if (inner.includes("/")) {
        ...
        if (numerator) {
            const items = numerator.split(' ');
            let pre = '';
            for (let item of items) {
                if (item == '[]') {
                    if (pre) data.push(pre);
                    data.push('[]');
                    pre = '';
    ...
    if (inner.includes("KT")) {
        if (inner.includes("KT[")) {//nếu có chứa number
            let regexGetNum = /\d+/g;
        ...
        } else {//nếu không chưa number
            const SPLIT = "KT:";
            let inner_slice: any = inner.slice(SPLIT.length, inner.length).split(";").map(item => item.trim());
    ...
    if (inner.includes("ND_1:") || inner.includes("ND_2:")) {
        let SPLIT = "ND_1:" || "ND_2:";
        let content_ND: any = [];
        inner.slice(SPLIT.length, inner.length).split('\n').map(item => {
            content_ND.push({
                type: ITEM_TYPE.TEXT,
                data: item
            });
...
export const splitStringBySpecialCharacter = (str: string, is_answer?: boolean) => {
    const result: Item[] = [];
    if (is_answer) {
